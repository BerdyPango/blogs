...
## Digital Picture Frame with Feh

```bash
$ feh --quiet --preload --randomize --zoom fill --full-screen --reload 60 --hide-pointer --slideshow-delay 15.0 /home/pi/Pictures/
```

- quiet: filter out error files
- preload: filter out unloadabale files
- randomize: slide pictures in random order
- zoom: `fill`, zoom the picture to window size
- full-screen: in full-screen mode
- reload: reload filelist and current image after `int` seconds. Useful for viewing HTTP webcams or frequently changing directories
- hide-pointer: hide the cursor
- slideshow-delay: intervals between 2 slides.
- draw-filename: add file name text on the left top corner
...
#!/bin/sh
feh --quiet --preload --randomize --scale-down --full-screen --reload 60 --hide-pointer --slideshow-delay 15.0 /home/pi/Pictures/
